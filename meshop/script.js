        ...
        if (result.code === "00") {
            ...
            bankSelect.innerHTML =
                ...
                banks
                    .map(
                        bank =>
                            `<option value="${bank.bin}">${bank.shortName}</option>`
                    )
                    .join("");
        } else {
...
async function generateVietQrFromForm() {
    const cacheKey = `${$("bankSelect").value}_${$("accountNo").value}_${
        $("outTotal").innerText
    }`;
    if (qrCache[cacheKey]) {
        await saveQrToIndexedDB(qrCache[cacheKey]);
        $("qrAmountPreview").textContent = $("outTotal").innerText;
        return;
    }
    // ... existing code ...
    ...
    try {
        ...
        if (!bank || !accountNo || !accountName) {
            throw new Error(
                "Vui lòng điền đầy đủ ngân hàng, số tài khoản và tên tài khoản."
            );
        }

        // Lấy tổng giá trị hóa đơn từ #outTotal
        const total =
            getProducts().reduce((sum, p) => sum + p.qty * p.price, 0) -Number($("paidAmount").value)  +
        ...
        // Tạo URL VietQR
        const qrUrl = `https://img.vietqr.io/image/${bank}-${accountNo}-compact2.png?amount=${total}&addInfo=${encodeURIComponent(
            memo || ""
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if (orderEl) {
            orderEl.addEventListener("input", async e => {
                ...
                request.onsuccess = () => {
                    const imgs = request.result
                        ? { flowerImg: request.result.data }
                        : {};
                    if (
            ...
            limitedHistory.forEach(inv => {
                ...
                const thumbSrc =
                    inv.images && inv.images.flowerImg
                        ? escapeHtml(inv.images.flowerImg)
                        : "";
                const thumbDisplay = thumbSrc ? "block" : "none";

                // Tính tổng giá trị hóa đơn
                const total =
                    inv.products.reduce((sum, p) => sum + p.qty * p.price, 0) +
                    (Number(inv.shippingFee) || 0);

                console.log(
                    `Rendering history item ${inv.timestamp}: thumbSrc=${thumbSrc}, total=${total}`
                ); // Debug
                ...
                li.innerHTML = `
                    ...
                    <div class="invoice-details">
                        <span class="history-shop-name">${escapeHtml(
                            ...
                            inv.customerPhone || "Không có SĐT"
                        )}</span>
                        <span class="history-date">${new Date(
                            inv.timestamp
                        ).toLocaleString("vi-VN")}</span>
                        <strong  class="small history-total"> ${toCurrency(
                            total
                        )} ₫</strong>
                    </div>
...
function createProductElement(p = {}) {
    ...
    el.innerHTML = `
        ...
        <input style="min-width: 10px;" class="productQty" placeholder="SL" type="number" min="1" max="999" maxlenght="3" value="${
            p.qty || ""
        }">
        <input class="productPrice" placeholder="Giá (VNĐ)" type="number" min="1" value="${
            p.price || ""
        }">
        ...
        request.onsuccess = () => {
            ...
            if (imgs.flowerImg) {
                ...
                document
                    ...
                    .forEach(img => {
                        img.style.border =
                            img.getAttribute("data-image") === imgs.flowerImg
                                ? "2px solid var(--accent)"
                                : "2px solid transparent";
                    });
            } else {
            ...
            if (outQr) {
                if (imgs.qrImg) {
                    ...
                    outQr.style.display = $("showQr").checked
                        ? "block"
                        : "none";
                } else {
                    outQr.src = "";
...
function updateInvoice() {
    ...
    $("outOrderId").innerText = $("orderId").value.trim().includes("#")
        ? ""
        : $("orderId").value.trim();
    ...
    if (shipping > 0) {
        ...
        r.innerHTML = `<div style="flex:1;">Phí ship</div><div style="width:90px;text-align:right"></div><div style="width:110px;text-align:right">${toCurrency(
            shipping
        )} ₫</div>`;
        out.appendChild(r);
        total += shipping;
    ...
    $("outRemaining").innerText = toCurrency(total-$("paidAmount").value) + " ₫";
...
function parseAndFillForm(text) {
    ...
    const mainRegex =
        /Mã ĐH:\s*(\S+)\s*Người nhận:\s*(.+?),\s*(\d{10})\s*Địa chỉ:\s*(.+?)\s*Dự kiến giao:/s;
    ...
    try {
        ...
        request.onsuccess = () => {
            if (!request.result) {
                store.put({
                    key: "flowerImg",
                    data: "assets/uploads/transparent.webp"
                });
                console.log("Set default flowerImg");
            }
        };
        request.onerror = () =>
            console.error("Error checking default flowerImg:", request.error);
        ...
        async function shareInvoice() {
            ...
            try {
                ...
                const imageFile = new File(
                    [imageBlob],
                    `invoice_${$("orderId")?.value || "order"}.png`,
                    {
                        type: "image/png"
                    }
                );
